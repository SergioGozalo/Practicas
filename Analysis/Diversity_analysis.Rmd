...
genes <- t(read.delim("functional.tables/EMOSE-GC_ICM_250bp_gene.length.SCG.Norm.counts.tbl", header = TRUE, sep = "\t", row.names = 1))
...
ncol(genes2)
genes2[,1:7876] <- sapply(genes2[, c(1:7876)], as.integer)
gene_ss <- rrarefy(genes2, min(rowSums(genes2)))
gene_acc <- specaccum(gene_ss, method = "exact", permutations = 1000)
plot(gene_acc)
```
